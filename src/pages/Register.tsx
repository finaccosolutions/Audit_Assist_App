...
export default function Register({ onToggle }: { onToggle: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error, email, requiresVerification } = await signUp(
      formData.email,
      formData.password,
      formData.fullName,
    ...
    if (error) {
      ...
      // Show success toast
      setToast({
        message: `Account created successfully! A verification email has been sent to ${email}. Please check your inbox and verify your email before signing in.`,
        type: 'success'
      });
        ...
        <div className="max-w-md w-full">
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-8 transform transition-all duration-300 hover:shadow-blue-500/20">
            ...
            <p className="text-center text-slate-300 mb-8">
              Start managing your auditing firm today
            </p>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full pl-11 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                    placeholder="Your Auditing Firm"
                    required
